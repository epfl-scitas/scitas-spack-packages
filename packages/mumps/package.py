...
class Mumps(Package):
    ...
    def write_makefile_inc(self):
        ...
        using_gcc_10plus = using_gcc and self.compiler.version >= version('10.0')
        ...
        if '+int64' in self.spec:
            if using_xlf:
                ...
                makefile_conf.extend([
                    'OPTF = %s -O  -DALLOW_NON_INIT %s %s' % (
                        ...
                        '-fallow-argument-mismatch' if using_gcc_10plus
                                                    else ''),  # noqa
                ])
            ...
            if using_xlf:
                ...
                makefile_conf.append('OPTF = %s -O%s -DALLOW_NON_INIT %s' % (
                    fpic, opt_level,
                    '-fallow-argument-mismatch' if using_gcc_10plus else ''))
