...
class Gcc(AutotoolsPackage, GNUMirrorPackage):
    ...
    # sm_30 is deprecated in cuda 11
    patch('nvptx-tools-cuda-arch.patch', when='^cuda@11.0.0:')
    ...
    def nvptx_install(self):
        ...
        if spec.satisfies('^cuda@11.0.0:'):
            options += [
                'CFLAGS=-DCUDA_ARCH=\"\\"sm_{}\\"\"'.format(60), # to replace with cuda_arch
                'CXXFLAGS=-DCUDA_ARCH=\"\\"sm_{}\\"\"'.format(60), # to replace with cuda_arch
            ]
