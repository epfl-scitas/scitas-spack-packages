...
class Charmpp(Package):
    ...
    # Charm++ < 7 does not have pmi support on the buildcmake script
    conflicts("pmi=simplepmi", when="@:6.9.99")
    conflicts("pmi=slurmpmi", when="@:6.9.99")
    conflicts("pmi=slurmpmi2", when="@:6.9.99")
    conflicts("pmi=pmix", when="@:6.9.99")
