...
class Libnl(AutotoolsPackage):
    ...
    patch('nvhpc-makefile.patch', when='%nvhpc')
    patch('nvhpc-hashtable.patch', when='%nvhpc') # sizeof(void) == 1 on gcc ?
    patch('nvhpc-pktloc.patch', when='%nvhpc') # __attribute((constructor)) returns void
