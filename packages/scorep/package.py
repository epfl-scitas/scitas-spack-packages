...
class Scorep(AutotoolsPackage):
    ...
    variant('cuda', default=False, description='Enable cuda tracing')
    variant('binutils', default=False, description='Enable libbfd support')
    ...
    depends_on('libiberty', when="+binutils")
    ...
    def configure_args(self):
        ...
        if "+cuda" in spec:
            config_args.extend([
                '--enable-cuda',
                '--with-libcudart-lib=%s' % spec['cuda'].prefix.lib64,
                '--with-libcudart-include=%s' % spec['cuda'].prefix.include,
                '--with-libcupti-lib=%s' % join_path(spec['cuda'].prefix,
                                                  'extras', 'CUPTI', 'lib64'),
                '--with-libcupti-include=%s' % join_path(spec['cuda'].prefix,
                                                         'extras', 'CUPTI',
                                                         'include')
            ])
        else:
        ...
        mpi_impl = {
            ...
            '^mpich@3:': 'mpich3',
            '^mvapich2@2.3:': 'mpich3',
            '^openmpi@1.7.5:1.99': 'openmpi',
            '^openmpi@2.0.0': 'openmpi3',
        }

        if '+mpi' in spec:
            for spec_ in mpi_impl:
                if spec.satisfies(spec_):
                    config_args.append('--with-mpi=%s' % mpi_impl[spec_])
        else:
